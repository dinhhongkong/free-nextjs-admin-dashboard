...
export default function LimousineSeat({disabledSeats, selectedSeats, setSelectedSeats} : SeatProps) {
  ...
  const renderSeats = (title: string, seatName: string, rows = 6, cols = 5) => {
    const seats = [];
    let seatCount = 1;
  ...
  return (
    <div className={"min-w-sm mx-auto flex w-[100%] max-w-2xl flex-col px-3 py-1 sm:px-6 2lg:mx-0 2lg:w-auto"}>
      ...
      <div className={"my-4 flex flex-row text-center font-medium gap-4 sm:gap-6 justify-center"}>
        {renderSeats("Tầng dưới", "A")}
        <div className={"max-w-2"}></div>
        {renderSeats("Tầng trên", "B")}
      </div>
    </div>
  );
}